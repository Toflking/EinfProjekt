...
// Methode für den Create Meal Screen, verwaltet alles, was dort vor sich geht
public class CreateMealController {

    // FXML Felder aus dem FXML file
    @FXML private ScrollPane root;
    @FXML private Label createMealStatusLabel;
    ...
    private void saveMeal() throws SQLException, IOException {
        ...
        String area = areaComboBox.getEditor().getText().trim();
        String thumb = thumbField.getText().trim();
        String youtube = youtubeField.getText().trim();
        ...
        if (name.isBlank() || instructions.isBlank()) {
            createMealStatusLabel.setText("Name and instructions cannot be empty");
            createMealStatusLabel.setStyle("-fx-text-fill: red;");
            return;
        }

        // Schauen, dass mindestens eine Zutat da ist
        if (ingredients.isEmpty()) {
            createMealStatusLabel.setText("There needs to be at least one ingredient");
            createMealStatusLabel.setStyle("-fx-text-fill: red;");
            return;
        }
        ...
        if (mealDAO.getMealByName(name) != null) {
            createMealStatusLabel.setText("Meal already exists");
            createMealStatusLabel.setStyle("-fx-text-fill: red;");
            return;
        }
        ...
        // user id setzen
        meal.setCreated_by_user_id(currentUser.getId());

        // Meal kreiren und id speichern
        int id = mealDAO.createMeal(meal);
        ...
        if (id > 0) {
           // für alle Ingredients ein Meal Ingredient erstellen
            for (MealIngredient mealIngredient : ingredients) {
                ...
                // Meal id setzen
                mealIngredient.setMeal_id(id);
                // Ingredient id setzen
                mealIngredient.setIngredient_id(ingredientDAO.getIngredientByName(mealIngredient.getIngredient_name()).getId());
    ...
    private void loadComboBoxData() {
        ...
        try {
            ...
            createMealStatusLabel.setText("Could not load form data");
            createMealStatusLabel.setStyle("-fx-text-fill: red;");
        }
    }

    // Methode zum Filtern der Comboboxen zu dem was im Feld steht
    private void configureFilterableComboBox(ComboBox<String> comboBox, List<String> allItems) {
